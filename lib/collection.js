...
/**
 * Wrapper around a single MongoDB collection.
 * Public methods are fail-silent: any thrown driver error is reported via
 * the parent client's observability hook and replaced with an empty default.
 */
export class Collection {
  ...
  async find (query, options) {
    try {
      ...
      return await col.find(prepare(query), buildFindOptions(options)).toArray();
    } catch (err) {
  ...
  async findOne (query, options) {
    try {
      ...
      this.client.emit(err, { method: 'findOne', collection: this.name, query });
      return null;
    }
  }
  ...
  async distinct (field, query) {
    try {
      ...
      this.client.emit(err, { method: 'distinct', collection: this.name, query });
      return [];
    }
  }
   ...
   * Returns the inserted documents with their `_id` populated, or [] on failure.
   * @param {object[]} docs
   * @returns {Promise<object[]>}
   */
  async saveAll (docs) {
    ...
    try {
      ...
      return prepared.map((d, idx) => ({ ...d, _id: d._id ?? result.insertedIds[idx] }));
    } catch (err) {
  ...
  async removeById (id) {
    try {
      ...
      this.client.emit(err, { method: 'removeById', collection: this.name, query: { _id: id } });
      return false;
    }
  }
}
